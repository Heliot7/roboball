...
GoalView::GoalView(const std::tr1::shared_ptr<World>& world):
    mpWorld(world),
    mpUniformModelViewProjectionMatrix(UniformMatrix4fControl().create())
{
}

void GoalView::updatResize(int w, int h)
{
    mWidth = w;
    mHeight = h;
    mAR = (float)w/(float)h;
...
void GoalView::setMatView(glm::vec3 trans, glm::vec3 scal)
{
...
void GoalView::paint(double time)
{

    setMatView(glm::vec3(0.0f,10.0f,0.0f),glm::vec3(5.0f*time,5.0f*time,0.0f));;
    mpUniformModelViewProjectionMatrix->setValue(matOrtho * matView );
    mGoal->render();
}

void GoalView::initView(SharedTexture sizeDepth, SharedTexture sizeColour) {

    mGoal = RenderObjectControl(
        ...
        ShaderProgramObjectControl(ShaderProgramFileManager::the()->get(ShaderProgramControlAutoFiles("Number"))).
            ...
            uniformTexture("uSamplerBaseColor",     UniformTextureControl(TextureFileManager::the()->get(TextureControlFileJPG("Goal.png"))).create()).
            create(),
